...
列表每行一条，支持 `#` 注释（`#` 后内容运行时自动去除）：

```
192.168.1.100  # 客厅 PC
192.168.1.101  # 卧室 NUC
```
...
```
# 命令模板（需先将公钥添加到目标主机 authorized_keys）
ssh -i /var/packages/runcmd/var/.ssh/id_rsa -o StrictHostKeyChecking=no user@{0} {1}
